...
mod state {
    ...
    impl State {
        pub fn from_states(
            containers: Vec<ui_lib::state::graph::Node>,
            assets: Vec<ui_lib::state::Asset>,
        ) -> Self {
...
fn partition_resources<'a>(
    resources: &'a Vec<ui_lib::state::workspace_graph::Resource>,
) -> (
    Vec<&'a ui_lib::state::workspace_graph::Resource>,
    Vec<&'a ui_lib::state::workspace_graph::Resource>,
) {
...
fn resources_to_update_args(
    resources: &Vec<ui_lib::state::workspace_graph::Resource>,
    graph: &ui_lib::state::Graph,
) -> (
    Vec<PathBuf>,
    Vec<lib::command::asset::bulk::ContainerAssets>,
