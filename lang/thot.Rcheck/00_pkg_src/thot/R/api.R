...
#' cat("Hello!", path)
add_asset <- function(db, file, name = NULL, tyfpe = NULL, tags = list(), metadata = list()) {
  ...
  asset <- json_asset(file, name = name, type = type, tags = tags, metadata = metadata)
  cmd <- sprintf('{"AssetCommand": {"Add": %s}}', asset)
