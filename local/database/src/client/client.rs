...
mod project {
    ...
    impl Client {
        ...
        pub fn get(&self, path: impl Into<PathBuf>) -> zmq::Result<Option<state::Project>> {
            let user = self.send(query::Project::Get(path.into()).into())?;
            Ok(serde_json::from_value(user).unwrap())
        }
        ...
        pub fn graph(
            &self,
            project: ResourceId,
        ) -> zmq::Result<Option<state::FolderResource<state::Graph>>> {
            let user = self.send(query::Project::Graph(project).into())?;
            Ok(serde_json::from_value(user).unwrap())
        }
