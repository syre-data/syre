...
impl Database {
    pub fn handle_query_project(&self, query: query::Project) -> JsValue {
        match query {
            ...
            query::Project::Graph(project) => {
                let state = self.handle_query_project_graph(&project);
                serde_json::to_value(state).unwrap()
            }
        }
    }
    ...
    /// # Returns
    /// Project's data graph.
    /// `None` if a state is not associated with the project.
    fn handle_query_project_graph(
        &self,
        project: &ResourceId,
    ) -> Option<state::FolderResource<state::Graph>> {
        self.state.projects().iter().find_map(|state| {
            ...
            if properties.rid() == project {
                Some(state.graph().map(|graph| graph.as_graph()))
            } else {
                None
            }
        })
