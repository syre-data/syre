...
use syre_project_daemon::constants;
...
fn publ() -> zmq::Result<()> {
    ...
    zmq_socket
        .bind(&syre_project_daemon::common::zmq_url(zmq::PUB).unwrap())
        .unwrap();
...
fn sub() -> zmq::Result<()> {
    ...
    zmq_socket.connect(&syre_project_daemon::common::zmq_url(zmq::SUB).unwrap())?;
    zmq_socket
    ...
    loop {
        ...
        match serde_json::from_str::<Vec<syre_project_daemon::event::Update>>(&message) {
            Ok(message) => println!(
                "{topic}\n{}\n",
...
fn req() -> zmq::Result<()> {
    ...
    zmq_socket.connect(&syre_project_daemon::common::zmq_url(zmq::REQ).unwrap())?;
...
fn rep() -> zmq::Result<()> {
    ...
    zmq_socket.bind(&syre_project_daemon::common::zmq_url(zmq::REP).unwrap())?;
