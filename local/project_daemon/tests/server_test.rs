...
fn test_server_state_and_updates_basics() {
    ...
    let db = syre_project_daemon::server::Builder::new(config);
    thread::spawn(move || db.run().unwrap());
    let db = syre_project_daemon::Client::new();
...
fn test_server_state_and_updates_graph() {
    ...
    let c1_graph_path =
        syre_project_daemon::common::container_graph_path(project.data_root_path(), c1.base_path())
            .unwrap();
    let container_state = db
        .container()
    ...
    let c2_graph_path =
        syre_project_daemon::common::container_graph_path(project.data_root_path(), c2.base_path())
            .unwrap();
    let container_state = db
        .container()
    ...
    let c2_graph_path =
        syre_project_daemon::common::container_graph_path(project.data_root_path(), &c2_path)
            .unwrap();
    let container_state = db
        .container()
        .get(project.rid().clone(), &c2_graph_path)
    ...
    assert_eq!(
        *from,
        syre_project_daemon::common::container_graph_path(
...
impl UpdateListener {
    pub fn new(tx: Sender<Vec<Update>>, port: PortNumber) -> Self {
        ...
        socket
            .set_subscribe(syre_project_daemon::constants::PUB_SUB_TOPIC.as_bytes())
            .unwrap();

        socket
            .connect(&syre_project_daemon::common::localhost_with_port(port))
            .unwrap();
